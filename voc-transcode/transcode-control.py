...
from config import TranscodeControlConfig
import mqtt
...
def transcodings_get_running():
    ...
    manager.load()
    transcodings = []
    for service in manager.Manager.ListUnitsByPatterns(
        ["active"], ["transcode@*.service"]
    ):
...
def transcodings_start(stream):
    ...
    transcoder_config = dict()
    transcoder_config["stream_key"] = stream
    transcoder_config["format"] = streaminfo["format"]
    transcoder_config["output"] = "direct"
    transcoder_config["type"] = "h264-only"
    transcoder_config["transcoding_source"] = streaminfo["source"]
    transcoder_config["transcoding_sink"] = config.sink

    try:
        passthrough_mode = cons.kv.get("stream/%s/passthrough" % stream)[1][
            "Value"
        ].decode()
    except:
        passthrough_mode = "no"
    transcoder_config["passthrough"] = passthrough_mode

    try:
        restream_target = cons.kv.get("stream/%s/restream" % stream)[1][
            "Value"
        ].decode()
    except:
        restream_target = "none"
    transcoder_config["restream"] = restream_target

    try:
        framerate = cons.kv.get("stream/%s/framerate" % stream)[1]["Value"].decode()
    except:
        framerate = 25
    transcoder_config["framerate"] = framerate

    transcoder_config_path = config.config_path + "/" + stream
    with open(transcoder_config_path, "w") as f:
        yaml.dump(transcoder_config, f)
...
def get_all_streams():
    ...
    for key in streamkeys:
        if ("stream/%s/transcoder" % key) in keys:
            try:
                streams[key] = (
                    cons.kv.get("stream/%s/transcoder" % key)[1]["Value"]
                    .decode()
                    .split(".")[0]
                )
            except:
...
def mainloop():
    ...
    if len(my_streams) > config.capacity:
        for stream in my_streams[config.capacity :]:
            log(
                "Transcoder is over capacity, stopping transcoding for stream %s"
                % stream
            )
            transcodings_stop(stream)
            try:
    ...
    if len(my_streams) < config.capacity:
        for stream, transcoder in get_all_streams().items():
            ...
            for i in range(5):
                time.sleep(
                    len(my_streams)
                )  # extremely professional load balancing hack
            ...
            if (
                cons.kv.get("stream/%s/transcoder" % stream)[1]["Value"].decode()
                != config.hostname
            ):
    ...
    cons.kv.put(
        "transcoders/%s" % config.hostname, json.dumps(transcoder_info).encode()
    )
...
def main():
    parser = argparse.ArgumentParser(description="Control transcode tasks")
    parser.add_argument(
        "--config",
        help="path to config file",
        default="/etc/transcode-control/config.toml",
    )
    args = parser.parse_args()
    global config
    config = TranscodeControlConfig(args.config)
    while True:
        try:
            mainloop()
        except:
